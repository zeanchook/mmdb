...
export default function GenreTabMovies() {
  ...
  let displayTitle = "";
  if (genreName) {
    let finderName = genreName?.records?.findIndex(
      (x) => parseInt(x.fields.id) === parseInt(id)
    );
  ...
  return (
    ...
    >
      ...
      >
        <button style={{ width: "100px" }} onClick={handleClickPrev}>
          Prev Page
        </button>
        <button style={{ width: "100px" }} onClick={handleClickNext}>
          Next Page
        </button>
      </div>
    </div>
