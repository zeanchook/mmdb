...
export default function MovieDetails({handleFavourite})
{
    ...
    // ! provider
        ...
        const selectorBox = () =>
        {
            ...
            return (<span>{regionList.length} provider available<div><label htmlFor="regionlist-choice">Check Provider : </label>
        ...
        const providerBarWatch = providerSelected.flatrate?.map((x,idx) => 
        {
            return (
                <div key={idx} style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center"}}>
                    <img src={`https://image.tmdb.org/t/p/original/${x.logo_path}` }style={{borderRadius:"10px",height:"50%",width:"50%"}}/>
                    <p style={{fontSize:"10px",textAlign:"center"}}>{x.provider_name}</p>
                </div>
            )
        })
...
console.log(movieDetails)
    ...
    <div style={{display:"flex",alignItems:"center",backgroundImage:`url(https://image.tmdb.org/t/p/original${movieDetails.backdrop_path})`,backgroundSize:"cover",backgroundPosition:"center",minHeight: "100vh"}}>
        <img className="main-poster-container"  src={`https://image.tmdb.org/t/p/w500${movieDetails.poster_path}`} style={{background:"grey",borderRadius:"10px"}} alt={movieDetails.original_title} />
    <div style={{margin:"10px"}}></div>
    <div className="movie-details" style={{color:"black",background:"white",borderRadius:"30px",opacity:"98%",padding:"20px",width:"900px"}}>
        <div className="movie-description">
            <h1 style={{display:"flex",alignItems:"center"}}>{movieDetails.title}
            <div style={{display:"flex",alignItems:"center",marginLeft:"10px"}}>{curvalue[1] === "yes" ? <img name="favourite" onClick={handleFavouriteClick} style={{width:"40px",height:"40px"}}src="https://cdn-icons-png.flaticon.com/128/2550/2550357.png"></img> : <img name="favourite" onClick={handleFavouriteClick} style={{widht:"40px",height:"40px"}}src="https://cdn-icons-png.flaticon.com/128/2550/2550223.png"></img> }</div>
            <div style={{display:"flex",alignItems:"center",marginLeft:"10px"}}>{curvalue[2] === "yes" ? <img name="wishlist" onClick={handleFavouriteClick} style={{width:"40px",height:"40px"}}src="https://cdn-icons-png.flaticon.com/128/10525/10525533.png"></img> : <img name="wishlist" onClick={handleFavouriteClick} style={{widht:"40px",height:"40px"}}src="https://cdn-icons-png.flaticon.com/128/10525/10525544.png"></img> }</div>
            <div style={{ width: 60, height: 60,display:"flex",alignItems:"center",marginLeft:"10px"}}><CircularProgressbar value={curvalue[0]}  maxValue={10} text={`${curvalue[0] === undefined ? "-": "⭐️ " + curvalue[0]}`}/></div>
            </h1>
            
           
            <div className="movie-title" style={{textAlign:"left"}}>{movieDetails.tagline}</div>
            <hr/>
            
            <div className="movie-title" style={{textAlign:"left"}}>Genre: {genres}</div>
            <hr/>
            
            <div className="movie-title" style={{textAlign:"left"}}>Release Date : {convertDateFormat(movieDetails?.release_date)}</div>
            <hr/>

            <div className="movie-title" style={{textAlign:"left"} }>Movie Description : {movieDetails?.overview}</div>
            <hr/>

            <div className="movie-title" style={{textAlign:"left"} }>Runtime : {minutesToHours(movieDetails?.runtime)}</div>
            <hr/>
        ...
        <div className="movie-buttons" > 
            ...
            {trailerDisplayStatus === true ? <ModalVideo 
                ...
                videoId={trailerID}
            /> : <span style={{marginLeft:"10px",color:"red",fontSize:"20px"}}>No trailer available ☹️</span>}
        ...
        <hr/>
        <div className="movie-title" style={{textAlign:"left"}}>Provider Status : {selectorBox()}
